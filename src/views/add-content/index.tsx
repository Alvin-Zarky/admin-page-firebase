...
import ImgFile from "../../assets/img/gallery.png";
...
export default function AddContent() {
  ...
  const { addContentAction, deleteContentAction, editContentAction } =
    useDispatchHook();
  ...
  const onFileUpload = async (e: any) => {
    ...
    try {
      ...
      const imagePath = storage.ref(
        `/images/${file.name}-${date.getTime().toString()}-${randomNumber}`
      );
                ...
                <Col xl="6" lg="6" md="6">
                  ...
                  <Modal isOpen={modal} toggle={onToggle}>
                    <form onSubmit={onSubmitContent}>
                        ...
                        <div className="box-input-modal">
                          ...
                          <div className="input-text">
                            ...
                            >
                              ...
                              {category &&
                                category.map(
                                  (val: DataCategory, ind: number) => (
                                    <option key={ind} value={val.categoryName}>
                                      {val.categoryName}
                                    </option>
                                  )
                                )}
                            ...
                            {pathImg ? (
                              <>
                                ...
                                <div className="style-bg-img">
                                  {pathImg && <img src={pathImg} alt="" />}
                                  {!pathImg && <img src={ImgFile} alt="" />}
                                </div>
                              </>
                            ) : (
                              <>
                      ...
                      <ModalFooter>
                        <button
                          className={`btn-submit ${isBlur && `btn-disabled`}`}
                        >
                          Submit
                        </button>
                        <Button
              ...
              <div className="table-content">
                <Table bordered>
                  ...
                  <tbody>
                    {isLoading && <Loading />}
                    {content &&
                      content.map((value: DataContent, ind: number) => (
                        <tr key={ind}>
                          <td>{value.id}</td>
                          <td>{value.categoryType}</td>
                          <td>
                            {(value.contentTitle as string).length > 30
                              ? `${value.contentTitle?.substring(0, 30)}...`
                              : `${value.contentTitle}`}
                          </td>
                          <td>
                            {(value.contentDescription as string).length > 30
                              ? `${value.contentDescription?.substring(
                                  0,
                                  30
                                )}...`
                          ...
                          <td>
                            <div className="button-edit-del">
                              ...
                              <div
                                ...
                                onClick={() => {
                                  onDeleteContent(value.id as string);
                                }}
                              >
                                <AiFillDelete />
                              </div>
