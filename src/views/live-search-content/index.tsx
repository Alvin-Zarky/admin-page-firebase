...
import { DataCategory, DataContent, getCategoryAction, searchContentAction } from '../../actions/interfaceAction'
import Loading from '../../components/Loading'
import { GrUpdate } from 'react-icons/gr'
import { AiFillDelete } from 'react-icons/ai'
import { Router } from '../../router'
import { storage } from '../../config/firebase'
import '../add-content/add-content.scss'

interface ParamKeySearch{
  keyword: string
}
export default function LiveSearchContent() {
  ...
  const {isLoading} = useSelectorHook(state => state.getContent)
  const {content, isError, message} = useSelectorHook(state => state.searchContent)
  const {addContentAction, deleteContentAction, editContentAction} = useDispatchHook()
  const dispatch= useDispatch()
  const history= useHistory()
  const {keyword} = useParams<ParamKeySearch>()
  ...
  useEffect(() =>{
    ...
    if(keyword){
      dispatch(searchContentAction(keyword) as any)
    }
